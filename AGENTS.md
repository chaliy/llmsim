...
- All commits require attribution to actual human users, never to coding agents or bots.
- Do NOT set GIT_AUTHOR_NAME, GIT_COMMITTER_NAME, or user.name to any AI/bot identity (e.g., Claude, Copilot, Cursor, GPT, bot).
- Do NOT add "Co-authored-by" trailers for AI tools.
- Do NOT include phrases like "generated by" or "authored by AI" in commits or PRs.
- Merge commits must use the real user as author, never agent identities.
- The SessionStart hook (`.claude/hooks/fix-git-identity.sh`) auto-corrects agent-like git identities using `GIT_USER_NAME`/`GIT_USER_EMAIL` environment variables as fallback.
...
walkthrough (the diff shows that). Add a Before / After with proof — CLI/API
output, logs, metrics, or screenshots for UI — whenever behavior changes.
